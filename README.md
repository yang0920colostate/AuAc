...
- `v2.0.0` (expeceted on 07/01/2021): `Keras-Tensoflow` as the backend (another option)
- `v3.0.0` (expected on 07/01/2021): a `Fortran` version of the pacakge will be published
...
4. Example scripts:
   ...
   - `run_example2.py`: demonsrate how to use part of the testing dataset from `Chiu_etal_GRL2020_AuAc.nc` for evaluating the performance of the `Standard()` model. `Chiu_etal_GRL2020_AuAc.nc` and all the other files can be found in the [ARM Archive](https://sso.arm.gov/arm/login?service=https%3A%2F%2Fiop.archive.arm.gov%2Farm-iop%2F0pi-data%2Fchiu%2Faceena-ml%2F) (with **free registration** using this [link](https://adc.arm.gov/discovery/#/account)).
